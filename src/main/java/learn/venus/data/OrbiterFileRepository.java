...
public class OrbiterFileRepository implements OrbiterRepository {
    ...
    public List<Orbiter> findAll() throws DataAccessException {
        ...
        try(BufferedReader reader = new BufferedReader(new FileReader(filepath))) {
            ...
            for (String line = reader.readLine(); line != null; line = reader.readLine()) {
               String[] fields = line.split(",", -1);//-1 means to continue to consume all fields- don't stop
               if (fields.length == 4) {
    ...
    public Orbiter add(Orbiter orbiter) throws DataAccessException {
        ...
        int nextId = 0;
        for(Orbiter o: all) {
            nextId = Math.max(nextId, o.getOrbiterId());//max id between nextID and currentId
        }
        nextId++;
